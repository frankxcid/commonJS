...
/*global COMMON, AJAXPOST, FILLIN, window*/
...
TVT.populateVideoLinks = function (displayDivId, folderName) {
    ...
    "use strict";
    var baseObj = document.getElementById(displayDivId);
    var groupName = baseObj.getElementsByTagName("h5")[0].innerHTML;
    TVT.zgetLinks(folderName);
    if (!TVT.zvideodata || TVT.zvideodata.length === 0) { return; }
    TVT.zvideodata.forEach(function (item) {
        var iHTML = "<a href=\"trainingvideos.html?foldername=" + encodeURIComponent(folderName) + "&filename=" + encodeURIComponent(item.filename) + "&groupname=" + groupName + "\" >";
        iHTML += item.filename.replace(".mp4", "") + "</a>";
        var obj1 = COMMON.getBasicElement("div", null, iHTML, "onecell");
...
TVT.zinitdisplay = function (displayDivId) {
    ...
    if (TVT.zvideodata !== undefined && TVT.zvideodata !== null && TVT.zvideodata.length > 1) {
        TVT.zvideodata.forEach(function (item, index) {
            if (item.filename !== TVT.currentDisplayedFile) {
                var iHTML = "<a href=\"trainingvideos.html?foldername=" + encodeURIComponent(TVT.zsearchObj.foldername) + "&filename=" + encodeURIComponent(item.filename) + "&groupname=" + TVT.zsearchObj.groupname + "\" >";
                iHTML += "<img src=\"images/" + encodeURIComponent(item.filename.replace(".mp4", ".jpg")) + "\" alt=\"video thumbnail\" width=\"75\" height=\"75\" /><br />";
                iHTML += "<span>" + item.filename.replace(".mp4", "") + "</span></a>";
                obj1 = COMMON.getBasicElement("div", null, iHTML, "onecell");
                baseObj.appendChild(obj1);
            }
            if (item.filename === TVT.currentDisplayedFile) { TVT.zcurrentDisplayedFileIndex = index; }
        });
    } else {
...
TVT.zvideoStarted = function () {
    ...
    var obj = document.getElementById(TVT.zvidEnvelopeId);
...
TVT.zshowFullScreenButton = function () {
    ...
    var fadeValue = -1 * Math.sin((((2 * Math.PI / 17.5) * 0.1) - ((2 * Math.PI) / 17.5) * TVT.zfadeIteration) * Math.PI / 180);
...
TVT.zhideFullScreenButton = function () {
    "use strict";
    var btnObj = document.getElementById(TVT.zfullScreenBtnId);
    if (!btnObj) { return; }
...
TVT.zsetFullScreen = function () {
    "use strict";
    var leftDisplayObj = document.getElementById(TVT.zvidEnvelopeId);
...
TVT.zsetVideoObjMouseAttributes = function (remove) {
    "use strict";
    var vidObj = document.getElementById(TVT.zvidObjId);
    if (!remove) {
...
TVT.zmakeVideoDivVisible = function () {
    "use strict";
    var vidEnvelope = document.getElementById(TVT.zvideoEnvelopeId);
    var titleOffset = ((TVT.currentVideoTitle && TVT.currentVideoTitle !== "") ? 25 : 0);
    vidEnvelope.style.visibility = "visible";
    var wdOffset = window.innerWidth - TVT.zcurrentVideoWD;
    var htOffset = window.innerHeight - TVT.zcurrentVideoHT;
...
TVT.zgetDisplayRatio = function (vidObj) {
    ...
    var currentVideo = TVT.zvideodata[TVT.zcurrentDisplayedFileIndex];
    var ratio = currentVideo.frameWidth / currentVideo.frameHeight;
    var adjHeight = window.innerHeight; //height is the same in either configuration
    var adjWidth;
    if (TVT.zfullScreenStatus) {
...
TVT.zshowVideo = function () {
    ...
    var currentVideo = TVT.zvideodata[TVT.zcurrentDisplayedFileIndex];
    var vidEnvelope = document.getElementById(TVT.zvidEnvelopeId);
    ...
    var obj1 = document.createElement("source");
    ...
    var obj2 = COMMON.getBasicElement("h2", null, "Loading video...Please Wait.");
