...
FILLIN.yesNoDialog = function (parentDivId, headLine, message, width, continuingFunction, optionalData) {
    ...
    ///<param name="message" type="String|Element">(Optional) message at the top of the form main area. if String adds a div with string as content. If object, appends as child</param>
    ///<param name="width" type="String">Any valid css width value</param>
    ///<param name="continuingfunction" type="function">(Optional) a function that will be used when a user clicks a button in the dialog. the function should be assigned to a variable and have the pattern: function (dialogResult, dataValuesFromDialog, optionalData) where dialogResult is the value assigned in the button Definition, dataValuesFromDialog is an object that contains the values from the controls. if there are no values (such as dataValuesFromDialog is empty) then optionalData will be the value in order to accomodate continuing functions with only 2 arguments when the Dialog is known not to have values, and optionalData is the value of optionalData from the Dialog object. See dataValuesFromDialog pattern above.</param>
    ///<param name="optionalData" type="object">(Optional) Ignored if continuingfunction is not specified. Any data of any type the will be passed to the continuing function</param>
    "use strict";
    var formIndex;
